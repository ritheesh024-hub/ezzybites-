...
export const DashboardAnalysis = ({ orders, products }: DashboardAnalysisProps) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {/* FILTER BAR */}
      <div className="flex flex-col lg:flex-row justify-between items-center gap-4 bg-white p-3 rounded-[2rem] shadow-sm border">
        <div className="flex bg-secondary/30 p-1 rounded-full w-full lg:w-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 rounded-[2.5rem] border-none shadow-xl bg-white p-6">
      ...
      <Dialog open={!!activeMetricView} onOpenChange={() => setActiveMetricView(null)}>
        <DialogContent className="max-w-3xl rounded-[2.5rem] bg-white border-none shadow-3xl p-10">
          <DialogHeader>
            <DialogTitle className="text-3xl font-black font-headline uppercase tracking-tighter flex items-center gap-3">
          ...
          <div className="mt-8 grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-secondary/20 rounded-[2rem] p-6 flex flex-col items-center justify-center text-center">
...
const MetricCard = ({ label, value, icon: Icon, color, onClick }: any) => (
  <Card 
    onClick={onClick}
    className="rounded-[2rem] border-none shadow-xl bg-white p-6 hover:scale-[1.03] transition-all cursor-pointer group"
  >
